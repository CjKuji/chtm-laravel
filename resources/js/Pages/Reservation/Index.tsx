...
export default function ReservationPage() {
  return (
    <RequireRole allowedRoles={['reservation', 'super_admin', 'admin']}>
...
function ReservationPageInner() {
  ...
  // Tabs that depend on rejection workflow are intentionally removed per policy.
  // Keeping state type intact; UI will no longer expose the rejected tab.
  ...
  const handleEditBooking = useCallback(
    async (id: number, payload: any) => {
      await editBooking(id, payload);
      await refresh();
      await refreshSelected(id);
    },
    [editBooking, refresh, refreshSelected]
  );
      ...
      >
        <div className="space-y-6 p-4 sm:p-6">
          ...
          <div>
            ...
            {/* Rejection workflow removed: booking transitions to Confirmed/Approved only after payment verification. */}
          </div>
