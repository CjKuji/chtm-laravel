...
export default function ProfilePage() {
  ...
  const extractLastName = (value?: string | null) => {
    if (!value) return '';
  ...
  const fetchUser = async () => {
    ...
    const userProfile: User = {
      ...
      lname: extractLastName(profileData?.lname || ''),
  ...
  const handleSave = async () => {
    ...
    const trimmedLName = extractLastName(formLName.trim());
  ...
  return (
    <div className={`flex min-h-screen bg-gray-50 font-sans antialiased font-sans`}>
      <Sidebar activeMenu="profile" />
      <main className="flex-1 ml-64">
        <Topbar />

        <div className="p-6 max-w-2xl mx-auto">
          <h1 className={`text-2xl font-bold text-gray-800 mb-6 font-serif`} style={{ color: '#3D5A4C' }}>
            My Profile
          </h1>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div className="px-6 py-8 text-white" style={{ background: 'linear-gradient(135deg, #3D5A4C 0%, #2d4339 100%)' }}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className={`text-2xl font-bold font-serif`}>{user.fname} {user.lname}</h2>
              ...
              {!isEditing ? (
                <>
                  <div className="space-y-4">
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>Full Name</label>
                      <p className="text-lg text-gray-800">{user.fname} {user.lname}</p>
                    </div>
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>Email Address</label>
                      <p className="text-lg text-gray-800">{user.email}</p>
                    </div>
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>Role</label>
                  ...
                  <div className="mt-6 flex gap-3">
                    ...
                    <button onClick={() => window.history.back()} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50">Back</button>
                  </div>
                </>
              ) : (
                <>
                  <div className="space-y-4">
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>First Name</label>
                      <input value={formFName} onChange={e => setFormFName(e.target.value)} className="w-full px-3 py-2 border rounded-lg" />
                    </div>
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>Last Name</label>
                      <input value={formLName} onChange={e => setFormLName(e.target.value)} className="w-full px-3 py-2 border rounded-lg" />
                    </div>
                    <div>
                      <label className={`block text-sm font-medium mb-1 font-serif`} style={{ color: '#3D5A4C' }}>Email</label>
