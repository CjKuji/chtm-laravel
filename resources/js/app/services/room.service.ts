...
export const RoomService = {
  ...
  async getRooms() {
    const [roomsRes, typesRes] = await Promise.all([
      supabase
        .from("rooms")
        .select(`*, room_types (id, name, capacity, base_price, description, room_amenities (amenities (id, name))), room_images (id, image_url, display_order)`)
  ...
  async getRoomById(id: number) {
    const [roomRes, typesRes] = await Promise.all([
      supabase
        .from("rooms")
        .select(`*, room_types (*, room_amenities (amenities (id, name))), room_images (*)`)
        .eq("id", id)
        .maybeSingle(),
  ...
  async updateRoomFlags(id: number, payload: any) {
    const { data, error } = await supabase
      .from("rooms")
      .update(payload)
      .eq("id", id)
    ...
    if (error) throw new Error(`[updateRoomFlags] ${error.message}`);
    return data;
  },
